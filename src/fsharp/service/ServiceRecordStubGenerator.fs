...
module internal RecordStubGenerator =
    open Microsoft.FSharp.Compiler.SourceCodeServices.CodeGenerationUtils
            ...
            | _ ->
                ...
                let indentColumn =
                    ...
                    |> List.pick (fun (_, indentColumn, lineIdx) ->
                        match maxLineIdx with 
                        | Some idx when idx = lineIdx -> Some indentColumn
                        | _ -> None
                    )
    ...
    let formatRecord (insertionPos: RecordStubsInsertionParams) (fieldDefaultValue: string)
        ...
        let ctxt =
            { RecordTypeName = entity.DisplayName
              RequireQualifiedAccess = Symbol.hasAttribute<RequireQualifiedAccessAttribute> entity.Attributes 
              Writer = writer
    ...
    let tryFindRecordBindingInParsedInput (pos: pos) (parsedInput: ParsedInput) =
        ...
        and walkExpr expr =
            getIfPosInRange expr.Range (fun () ->
                ...
                | SynExpr.Sequential (_,_,synExpr1,synExpr2,_range) -> 
                    List.tryPick walkExpr [synExpr1; synExpr2]
